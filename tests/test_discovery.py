...
def _prepare_versions(pyenv_root, versions, expected_version=None):
    ...
    for version in versions:
        ...
        # If version is e.g. 3.6.12, we want to create python, python3, and python3.6
        for i in range(min(len(components), 2)):
            bin_path = bin_dir / ('python' + '.'.join(components[:i]))
...
@pytest.mark.parametrize('versions,requested_versions,expected_version', [
    (['3.6.10'], ['py36'], '3.6.10'),
    (['3.6.2', '3.6.10'], ['py36'], '3.6.10'),
    (['3.7.11'], ['py37'], '3.7.11'),
    (['3.8.11'], ['py38'], '3.8.11'),
    (['3.9.11'], ['py39'], '3.9.11'),
    (['3.10.11'], ['py310'], '3.10.11'),
    (['3.11.11'], ['py310'], '3.11.11'),
])
def test_cpython_abbrev_ok(
    pyenv_root, options_mock, python_info_mock, from_exe_mock,
    versions, requested_versions, expected_version,
):
    options_mock.python = requested_versions
